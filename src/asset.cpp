...
void load_assets(AssetState * assets, MemoryArena * arena) {
	...
	{
		u32 tex_size = 64;
		u8 * tex_data = PUSH_MEMORY(assets->arena, u8, tex_size * tex_size * TEXTURE_CHANNELS);
		for(u32 y = 0, i = 0; y < tex_size; y++) {
			for(u32 x = 0; x < tex_size; x++, i += TEXTURE_CHANNELS) {
				tex_data[i + 0] = 255;
				tex_data[i + 1] = 255;
				tex_data[i + 2] = 255;
				tex_data[i + 3] = 255;
		...
		gl::Texture gl_tex = gl::create_texture(tex_data, tex_size, tex_size, GL_RGBA, GL_LINEAR, GL_CLAMP_TO_EDGE);

		Asset * asset = push_asset(assets, AssetId_white, AssetType_texture);
		asset->texture.dim = math::vec2(tex_size);
